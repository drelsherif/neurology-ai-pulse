...
<html lang="en">
  <head>
    ...
    <script type="module" crossorigin src="./assets/index-B8Tup0Vs.js"></script>
    <link rel="stylesheet" crossorigin href="./assets/index-CZ19uc8N.css">
  </head>
  <body>
    <div id="root"></div>
...
(function() {
  ...
  // Only ever runs on mobile — never touches desktop at all
  if (window.innerWidth >= PAPER_W + PAD * 2) return;

  // Bundle DOM structure:
  //   scrollContainer: flex:1, overflow:auto, flexDirection:column,
  //                    alignItems:CENTER, padding:"24px 0 80px"
  //     scaleWrapper:  width:800, transform:scale(1), transformOrigin:"top center"
  //       #nap-paper:  width:800

  // Strategy:
  //   - Scale and reposition the scaleWrapper only
  //   - NEVER touch scrollContainer (breaks desktop centering and scroll)
  //   - Fix blank space by adding a negative-margin spacer AFTER the wrapper
  //     (not on the wrapper itself, which breaks scroll)

  var spacer = null; // injected once, updated each time

  function fix() {
    var paper = document.getElementById('nap-paper');
    if (!paper) return;

    var vw    = window.innerWidth;

    // Re-check in case device rotated to landscape
    if (vw >= PAPER_W + PAD * 2) {
      // Remove everything we set and stop
      var sw = paper.parentElement;
      if (sw) {
        sw.style.removeProperty('width');
        sw.style.removeProperty('transform');
        sw.style.removeProperty('transform-origin');
        sw.style.removeProperty('margin-left');
      }
    ...
    var scale      = (vw - PAD * 2) / PAPER_W;
    var scaledW    = Math.round(PAPER_W * scale);
    var scaleWrapper = paper.parentElement;

    if (scaleWrapper) {
      // Tell layout this wrapper is only scaledW wide (not 800px)
      scaleWrapper.style.width           = scaledW + 'px';
      // Scale from top-left — content stays at left edge
      scaleWrapper.style.transformOrigin = 'top left';
      scaleWrapper.style.transform       = 'scale(' + scale.toFixed(4) + ')';
      // The scrollContainer uses alignItems:center — with a scaledW wrapper
      // it will try to centre scaledW px, which is correct.
      // But transformOrigin:top-left means the visual content starts at the
      // left edge of the wrapper, not centred. Offset by the centering amount:
      var centerOffset = (vw - scaledW) / 2;
      scaleWrapper.style.marginLeft = '-' + centerOffset + 'px';
    }

    // Fix blank space: transform:scale leaves the original height in flow.
    // We insert a spacer with a negative top margin right after the wrapper
    // to cancel that dead space, WITHOUT touching the wrapper (which breaks scroll).
    var paperH   = paper.offsetHeight;
    var blankPx  = Math.round(paperH - paperH * scale);

    if (!spacer) {
      spacer = document.createElement('div');
      spacer.id = '__nap_spacer';
    }
    spacer.style.cssText = 'display:block;height:0;margin-top:-' + blankPx + 'px;';

    // Insert spacer right after the scaleWrapper if not already there
    if (scaleWrapper && scaleWrapper.parentNode) {
      if (spacer.parentNode !== scaleWrapper.parentNode ||
          spacer.previousSibling !== scaleWrapper) {
        scaleWrapper.parentNode.insertBefore(spacer, scaleWrapper.nextSibling);
      }
    }
  ...
  function watch() {
    if (document.getElementById('nap-paper')) { fixWithRetries(); return; }
    var obs = new MutationObserver(function() {
      if (document.getElementById('nap-paper')) {
        obs.disconnect();
        fixWithRetries();
      }
    });
